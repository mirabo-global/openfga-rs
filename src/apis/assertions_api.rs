...
pub async fn read_assertions(
    ...
    let uri_str = format!(
        ...
        authorization_model_id = crate::apis::urlencode(p_path_authorization_model_id)
    );
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => Err(Error::from(serde_json::Error::custom(
                "Received `text/plain` content type response that cannot be converted to `models::ReadAssertionsResponse`",
            ))),
            ContentType::Unsupported(unknown_type) => {
                Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ReadAssertionsResponse`"
                ))))
            }
        }
    } else {
